...
df_train_anWY=df_train_anWY[df_train_anWY['site_no'].isin(df_train_expl['STAID'])]
...
df_valin_anWY=df_valin_anWY[df_valin_anWY['site_no'].isin(df_valin_expl['STAID'])]
...
df_valnit_anWY=df_valnit_anWY[df_valnit_anWY['site_no'].isin(df_valnit_expl['STAID'])]
...
testreg=Regressor(expl_vars=df_train_mnexpl.drop(columns=['STAID']),
    resp_var = df_train_mnanWY['Ann_WY_ft3']/(df_train_mnexpl['DRAIN_SQKM']*(3280.84**2)),
...
pred_lasso_nit_mae = sum(np.abs(pred_lasso_nit - (df_valnit_mnanWY['Ann_WY_ft3']/(df_valnit_mnexpl['DRAIN_SQKM']*(3280.84**2)))))/len(pred_lasso_nit)
pred_lasso_nit_rmse = np.sqrt(sum((pred_lasso_nit - (df_valnit_mnanWY['Ann_WY_ft3']/(df_valnit_mnexpl['DRAIN_SQKM']*(3280.84**2))))**2)/len(pred_lasso_nit))
