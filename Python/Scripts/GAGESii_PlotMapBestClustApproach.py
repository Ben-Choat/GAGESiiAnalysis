...
Script to plot a map of which clustering approach
...
cmap_in = ListedColormap(['orange', 'lime', 'blue'])
# cmap_in = 'magma'
...
# prep data
##########
order_in = ['Eco', 'Class', 'None'] # for defining plotting order
# make points from lat long in id files
# valnit
# mean annul
...
geo_df_valnit_mannual['Region'] = geo_df_valnit_mannual['Region'].replace(
                                                    {'AggEcoregion': 'Eco'}
                                                    )
geo_df_valnit_mannual['Region'] = pd.Categorical(
                geo_df_valnit_mannual['Region'], 
                categories=order_in,
                ordered=True
                )
...
geo_df_valnit_annual['Region'] = geo_df_valnit_annual['Region'].replace(
                                                    {'AggEcoregion': 'Eco'}
                                                    )
geo_df_valnit_annual['Region'] = pd.Categorical(
                geo_df_valnit_annual['Region'], 
                categories=order_in,
                ordered=True
                )
...
geo_df_valnit_monthly['Region'] = geo_df_valnit_monthly['Region'].replace(
                                                    {'AggEcoregion': 'Eco'}
                                                    )
geo_df_valnit_monthly['Region'] = pd.Categorical(
                geo_df_valnit_monthly['Region'], 
                categories=order_in,
                ordered=True
                )
## %%
...
geo_df_valnit_monthly.plot(
    ax = ax3,
    column = 'Region', 
    markersize = 15, 
    marker = 'o',
    legend = False,
    cmap = cmap_in,# cmap_in,
    # vmin = -1,
    # vmax = 1,
    edgecolor = 'k',
    linewidth = 0.5,
...
# order_in = ['Eco', 'Class', 'None']
hue_order_in = ['regr_precip', 'strd_mlr', 'XGBoost']
xticklabs_in = order_in # mannual_count.index.str.replace('AggEcoregion', 'Eco')
...
if save_fig: 
    plt.savefig(
        ...
        f'{dir_figs}/NSE_Map_BestGroupingMeth_{name_in}.png',
        dpi = 300,
        bbox_inches = 'tight'
        )
