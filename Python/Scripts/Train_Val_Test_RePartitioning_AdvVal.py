...
# NOTE: Later (11/29/22) decided not to use daily data in analysis, so we were no longer
# limited to the catchments that dialy data succesfully downloaded for ... so
# several lines removing such catchments are commented out below
...
# define list of 8 catchments that are pracically duplicated based
# on explanatory variables but have different water yield values
...
# convert relevant expl vars to SI
# in/hr -> cm/hr
df_expl_all['PERMAVE'] = df_expl_all['PERMAVE'] * 2.54
# ft -> m
df_expl_all['WTDEPAVE'] = df_expl_all['WTDEPAVE'] * 3.28084
# in -> cm
df_expl_all['ROCKDEPAVE'] = df_expl_all['ROCKDEPAVE'] * 2.54


# drop 1998 since 1997 is not available for antecent forcings
df_expl_all = df_expl_all[df_expl_all['year'] != 1998]


# calculate mean variables
df_expl_all_mn = df_expl_all.groupby('STAID').mean().reset_index()

# The following lines that are commented out were initially used to 
# resave what was previously 'training' data to 'all data'
...
# read in staids to be removed due to high overlap and
# remove them from df_ID
staid_rm = pd.read_csv(
    f'{dir_expl}/STAID_ToRemove.csv',
    dtype = {0: 'string'}
)

df_ID = df_ID[
    -df_ID['STAID'].isin(staid_rm['0'])
    ]

# THIS SECTION IS ONLY APPLICABLE IF DAILY DATA WAS BEING USED, BUT 
# WE DECIDED AGAINST IT, SO THOSE CATCHMENTS THAT ARE FILTERED OUT 
# BELOW, NO LONGER NEED TO BE, THUS, IT IS COMMENTED OUT

# # read in list of STAIDs that DAYMET API failed to download data for
# daily_fails = pd.read_csv(
#     #'D:/DataWorking/Daymet/Daymet_Daily/DAILY_DMT_STILLNEEDED_TEMP.csv',
#     # dtype = {'STAID': 'string'}
...
#     'D:/DataWorking/Daymet/Daymet_Daily/IncompleteCatchments.csv',
...
# Of the ~ 3200 70% goes to training
...
for rs in np.arange(100, 1100, 100):
    ...
    X_tr, X_other = train_test_split(
        ...
        stratify = df_ID['HLR'] # 'AggEcoregion
    )
...
# drop 1998 since '97 isn't available for antecendent forcings
df_dmt_annual = df_dmt_annual[df_dmt_annual['year'] != 1998]
...
df_dmt_monthly = pd.read_parquet(
    'D:/DataWorking/Daymet/Daymet_monthly_GAGES.parquet'
)

# drop 1998 since '97 isn't available for antecendent forcings
df_dmt_monthly = df_dmt_monthly[df_dmt_monthly['year'] != 1998]
...
# define conversion factor for going from acre-ft to ft km2-ft to km2-cm using sqkm area value
# 247.105 acres in 1 km2
# 30.48 cm in ft
vol_conv = 30.48/247.105
...
# subset to years '99 - '12
df_wy_annual = df_wy_annual[
    df_wy_annual['yr'].isin(np.arange(1999, 2013, 1))
]
...
df_wy_annual_train['Ann_WY_cm'] = np.round(
    df_wy_annual_train['Ann_WY_acft'] * vol_conv/df_wy_annual_train['DRAIN_SQKM'],
    2
    )
...
df_wy_annual_valnit['Ann_WY_cm'] = np.round(
    df_wy_annual_valnit['Ann_WY_acft']/df_wy_annual_valnit['DRAIN_SQKM'] * vol_conv,
    2,
    )
...
df_wy_monthly = df_wy_monthly[
    df_wy_monthly['yr'].isin(np.arange(1999, 2013, 1))
    ]
...
df_wy_monthly_train['Mnth_WY_cm'] = np.round(
    df_wy_monthly_train['Mnth_WY_acft']/df_wy_monthly_train['DRAIN_SQKM'] * vol_conv,
    2
    )
...
df_wy_monthly_valnit['Mnth_WY_cm'] = np.round(
    df_wy_monthly_valnit['Mnth_WY_acft']/df_wy_monthly_valnit['DRAIN_SQKM'] * vol_conv,
    2
    )
