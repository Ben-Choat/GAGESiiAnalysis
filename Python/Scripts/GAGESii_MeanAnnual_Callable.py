...
# # training
# train_resp = df_train_anWY.groupby(
#     'site_no', as_index = False
# ).mean().drop(columns = ["yr"])
# # val_in
# valin_resp = df_valin_anWY.groupby(
#     'site_no', as_index = False
# ).mean().drop(columns = ["yr"])
# # val_nit
# valnit_resp = df_valnit_anWY.groupby(
#     'site_no', as_index = False
# ).mean().drop(columns = ["yr"])

# # mean GAGESii explanatory vars
# # training
# df_train_expl = df_train_expl.groupby(
#     'STAID', as_index = False
# ).mean().drop(columns = ['year'])
# # val_in
# df_valin_expl = df_valin_expl.groupby(
#     'STAID', as_index = False
# ).mean().drop(columns = ['year'])
# #val_nit
# df_valnit_expl = df_valnit_expl.groupby(
#     'STAID', as_index = False
# ).mean().drop(columns = ['year'])
...
# # training ID
# train_ID = df_ID[df_ID.STAID.isin(df_train_expl.STAID)].reset_index(drop = True)
# # val_in ID
# valin_ID = train_ID
# # val_nit ID
# valnit_ID = df_ID[df_ID.STAID.isin(df_valnit_expl.STAID)].reset_index(drop = True)
...
# %% Define input dataframes from 'Cluster_callPred.py'

# df_train_expl: training explanatory variables, replace 'train' with 'valin' or 'valnit' for those data
# train_resp: training response variables, replace 'train' with 'valin' or 'valnit' for those data
# train_ID: training ID variables, replace 'train' with 'valin' or 'valnit' for those data
"""
...
def regress_fun(df_train_expl, # training data explanatory variables. Expects STAID to be a column
    ...
    model_in = input('enter name for model (e.g., regr_precip):') # regr_precip'
    parameters = 'none'
    n_features = 1
    ...
    model_in = input('enter name for model (e.g., raw_lasso):') 
...
###############################################
    ...
    # model name for saving in csv
    model_in = input('enter name for model (e.g., forwardklim81):') 
    ...
    resp_in = train_resp['Ann_WY_ft']

    # define id vars
    id_in = train_ID['AggEcoregion']
    ...
    mdl_in = LinearRegression().fit(
                # df_train_expl[features_in], train_resp['Ann_WY_ft']
                expl_in, resp_in
                )
    ...
    # define response variable
    resp_in = valin_resp['Ann_WY_ft']

    # define id vars
    id_in = valin_ID['AggEcoregion']
    ...
    # define response variable
    resp_in = valnit_resp['Ann_WY_ft']

    # define id vars
    id_in = valnit_ID['AggEcoregion']
    ...
    # Write results to csv
    df_results.to_csv(f'{dir_expl}/Results/Results_NonTimeSeriesTEMP.csv', index = False)
    ...
    # model name for saving in csv
    model_in = input('enter name for model (e.g., strd_lasso):') 
    ...
    # model name for saving in csv
    model_in = input('enter name for model (e.g., stdrd_PCA_lasso):') 
    ...
    # model name for saving in csv
    model_in = input('enter name for model (e.g., stdrd_UMAP_lasso):') 
    ...
    # define model name and parameter name(s) to be written to files and used in file names
    # model name for saving in csv
    model_in = input('enter name for model (e.g., stdrd_UMAP_mlr):') 
    klim_in = input('input the maximum number of variables you want to consider (0<klim<81): ')
    model_name = model_in
    ...
    # %% 
    # # UMAP -> MLR feature selection - 'backward'
    """
