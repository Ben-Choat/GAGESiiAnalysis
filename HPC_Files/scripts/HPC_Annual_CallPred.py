...
clust_meth_in = sys.argv[1] # 'AggEcoregion' # 'HLR' #
...
region_in = sys.argv[2] # 'MxWdShld' # '1' # 
# region_in = '-1' # '16' # 
...
# Water yield
train_resp_in = df_train_anWY.loc[
    df_train_anWY['site_no'].isin(cidtrain_in['STAID']), 'Ann_WY_cm'
    ].reset_index(drop = True)

valnit_resp_in = df_valnit_anWY.loc[
    df_valnit_anWY['site_no'].isin(cidvalnit_in['STAID']), 'Ann_WY_cm'
    ].reset_index(drop = True)
...
train_expl_in = pd.merge(
    train_expl_in, df_train_anDMT, 
    left_on = ['STAID', 'year'], 
    right_on = ['site_no', 'year']
    ).drop('site_no', axis = 1)
valnit_expl_in = pd.merge(
    valnit_expl_in, df_valnit_anDMT, 
    left_on = ['STAID', 'year'], 
    right_on = ['site_no', 'year']
    ).drop('site_no', axis = 1)
