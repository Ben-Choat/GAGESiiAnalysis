...
clust_meth_in =  sys.argv[1] # 'HLR' #   # 'None' #  #'AggEcoregion'#
...
region_in =   sys.argv[2] # '1' # 'NorthEast' # # 'All' #'SEPlains' #'MxWdShld' # 

# define number of cores to use for applicable processes
ncores = int(sys.argv[3]) # 4 # 1 # 
...
df_train_ID = pd.read_csv(f'{dir_expl}/ID_train.csv',
    dtype = {'STAID': 'string'}).drop('DRAIN_SQKM', axis =1)
# val_nit ID
...
# Water yield
train_resp_in = df_train_mnanWY.loc[
    df_train_mnanWY['site_no'].isin(cidtrain_in['STAID']), 'Ann_WY_cm'
    ].reset_index(drop = True)

valnit_resp_in = df_valnit_mnanWY.loc[
    df_valnit_mnanWY['site_no'].isin(cidvalnit_in['STAID']), 'Ann_WY_cm'
    ].reset_index(drop = True)


# explanatory variables
train_expl_in = df_train_mnexpl[
    df_train_mnexpl['STAID'].isin(cidtrain_in['STAID'])
    ].reset_index(drop = True)
valnit_expl_in = df_valnit_mnexpl[
    df_valnit_mnexpl['STAID'].isin(cidvalnit_in['STAID'])
    ].reset_index(drop = True)
