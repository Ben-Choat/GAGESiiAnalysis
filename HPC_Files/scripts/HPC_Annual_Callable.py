...
def regress_fun(df_train_expl, # training data explanatory variables. Expects STAID to be a column
    ...
    if model_name in models_in:
    	...
    	# write coefs and intercept to csv
            ...
            # expl_in[not_tr_in] = regr.expl_vars[not_tr_in] # THIS IS WRONG - writing train vars back to valint
            expl_in[not_tr_in] = df_valnit_expl.drop(['STAID', 'year'], axis = 1)[not_tr_in]
            resp_in = valnit_resp
            # resp_in = train_mnanWY_tr
            id_in = valnit_ID
